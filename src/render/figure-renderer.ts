/**
 * Renderer de las figuras 3D (modo "Figuras"): el pool de `InstancedMesh` de
 * figuras + sombras + el pool chico de bordes (`LineSegments`), su hot-loop de
 * colocación/oclusión y el oclusor de la mano. Vive separado de `ARScene` para que
 * ésta quede como orquestador delgado (governor, experiencias, bloom) y este
 * subsistema —el más pesado y el cuello de testeo— sea un colaborador con un
 * contrato claro: `frame(params)` por cuadro y los setters de material.
 *
 * Mantiene el invariante near-zero-alloc del repo: reusa `Matrix4`/`Quaternion`/
 * `Euler`/`Vector3` y structs `target`/`corner` preasignados; no aloca por frame
 * ni por figura.
 */
import {
  BoxGeometry,
...
/** Parámetros del cuadro (estado de la escena + controles relevantes a las figuras). */
export interface FigureFrameParams {
  hands: NormalizedLandmark[][];
  figure: FigureKind;
  mirrored: boolean;
  sizeScale: number;
  spin: number; // ángulo acumulado (rad)
  dt: number;
  time: number; // timestamp del rAF (ms), para gracia/lastSeen
  multiHand: boolean;
...
export class FigureRenderer {
  ...
  /** `spin` del frame en curso (lo usa `composeMatrix`, evita pasarlo en cadena). */
  private spin = 0;
  ...
  constructor(scene: Scene) {
    ...
    scene.add(this.shadows, this.figures);
  ...
  setEdgeColor(value: string): void {
    this.edgeMaterial.color.set(value);
  }

  /** Oculta todo el pipeline de figuras (al entrar en modo experiencia creativa). */
  hide(): void {
  ...
  private updateOccluder(
    ...
    for (const lm of hand) {
      ...
      const p = landmarkToScreenInto(this.screenScratch, lm, w, h, mirrored);
      sp[m].x = p.x;
      sp[m].y = p.y;
      m++;
    }
  ...
  /** Avanza un cuadro del modo "Figuras": colocación, suavizado, sombra, oclusión. */
  frame(p: FigureFrameParams, w: number, h: number): void {
    this.spin = p.spin;
    if (!this.figures.visible) this.figures.visible = true;

    const maxFigures = p.multiHand ? HARD_HAND_CAP : 1;
    let figuresDirty = false;
    let shadowsDirty = false;

    for (let i = 0; i < this.slots.length; i++) {
      const slot = this.slots[i];
      const allow = p.figure !== "none" && i < maxFigures;
      const hand = allow ? p.hands[i] : undefined;
      ...
      const t = resolvePlacement({
        onHand:
          anchor !== null || isHeld(slot.everSeen, slot.lastSeen, p.time, HAND_GRACE_MS),
      ...
      if (!slot.primed) {
        ...
        slot.y = this.target.y;
        slot.s = this.target.s;
        slot.smoother.filterInto(slot, p.dt);
      }
    ...
    this.shadows.visible = p.shadowEnabled;
    ...
    const hand0 = p.figure !== "none" ? p.hands[0] : undefined;
    ...
    if (hand0 && hasHand0 && p.occlusionEnabled && this.facingBack) {
      this.updateOccluder(hand0, w, h, p.mirrored);
    } else {
      this.occluderMesh.visible = false;
    }
  }
